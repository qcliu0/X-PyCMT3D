...
def _xcorr_win_(arr1, arr2, Tapering=True):
    """
    ...
    if len(A1) != len(A2):
        raise ValueError("Length of arr1(%d) and arr2(%d) must be the same"
                         % (len(A1), len(A2)))
    cc = np.correlate(A1, A2, mode="full")
    nshift = cc.argmax() - len(A1) + 1
    # Normalized cross correlation.

    #print("A1,A2:",(A1 ** 2).sum(),(A2 ** 2).sum(),len(A1),len(A2))
    max_cc_value = \
        cc.max() / np.sqrt((A1 ** 2).sum() * (A2 ** 2).sum())
...
def compute_envelope_matrix(denv, obsd, synt, dt, win_idx, taper_type):
    ...
    #print("istart_d,iend_d,istart_s,iend_s:",istart_d,iend_d,istart_s,iend_s,win_idx[0],win_idx[1])
