...
class Cmt3D(object):
    ...
    def calculate_variance(self):
        ...
        for meta, trwin in zip(self.metas, self.data_container.trwins):
            ...
            var_all_new += np.sum(0.5 * meta.prov["new_synt"]["v"]
                                  * meta.weights)
            obs_all += np.sum(0.5 * meta.prov["synt"]["d"] * meta.weights)

            var_all0 += np.sum(0.5 * meta.prov["synt"]["v"])
            var_all_new0 += np.sum(0.5 * meta.prov["new_synt"]["v"])
        ...
        for meta, trwin in zip(self.metas, self.data_container.trwins):
            ...
            meta.prov["synt"]["chi"] /= var_all0
            meta.prov["new_synt"]["chi"] /= var_all0
            #meta.prov["synt"]["chi"] **= 1#0.5
            #meta.prov["new_synt"]["chi"] **= 1#0.5
        ...
        logger.info(
            ...
            % (var_all / var_all, var_all_new / var_all, (var_all - var_all_new) / var_all * 100))
        logger.info("*" * 20)

        self.var_all = var_all
        self.var_all_new = var_all_new
        self.var_reduction = [1 - var_all_new0 / var_all0, 1 - var_all_new / var_all]
    ...
    def plot_summary(self, outputdir=".", figure_format="png",
        ...
        plot_util = PlotInvSummary(
            data_container=self.data_container, metas=self.metas, config=self.config,
            ...
            var_all=self.var_all, var_all_new=self.var_all_new,
            mode=mode)
    ...
    def plot_stats_histogram(self, outputdir=".", figure_format="png"):
        ...
        plot_util = PlotStats(self.data_container, self.metas, figname, self.var_reduction)
    ...
    def write_weight_log(self, filename):
        ...
        weights = {}
        for meta in self.metas:
            weights[meta.obsd_id] = (float)(meta.weights)
        #print(weights)
